...
(function( $ ) {
	...
	$.fn.bsDropDownFilter = function(options) {
		return this.filter(".dropdown-menu").each(function() {
			...
			$('<input type="search" class="form-control" placeholder="' + opts.label + '" style="width:96%; margin:0 auto" />')
				...
				.bind('keyup', function(e) {
                    if (e.key === 'Enter') {
                        var visible_items = $droplist.find('li:visible').not('.dropdown-filter').not('.disabled');
                        if (visible_items.length === 1) {
                            window.location.href = visible_items.find("a").attr("href");
                            return false;
                        }
                    } else {
